...
-- NOTE: Resurrection blocking disabled - causes companion state corruption after resurrection
-- that cannot be reliably fixed via Lua. Companions who fail Death's Door can be resurrected
-- during combat, which slightly undermines the balance but preserves game functionality.
...
function DeathsDoor.Kill(entity)
    ...
    -- NOTE: Resurrection blocking removed - it caused companion state corruption
    -- that couldn't be fixed after resurrection. Companions can now be revived during combat.
    _P("[DanseMacabre] Entity killed (resurrection allowed): " .. tostring(entity))
end
